...
# Ensure passwordless sudo for the session user
if [ "$USER" != "root" ]; then
...
# Ensure X11 socket directory ownership
mkdir -p /tmp/.X11-unix
chown root:root /tmp/.X11-unix
chmod 1777 /tmp/.X11-unix
rm -f /tmp/.X1-lock /tmp/.X11-unix/X1

# Pre-create Xauthority (avoids xauth warnings)
...
# Keep a copy for TurboVNC default lookup
ln -sf "$(basename "${XSTART}")" "${HOME_DIR}/.vnc/xstartup.turbovnc"

# vnc run script
VNC_RUN="${HOME_DIR}/.vnc/vnc_run.sh"
cat > "${VNC_RUN}" <<EOF
#!/bin/sh
/opt/TurboVNC/bin/vncserver ${DISPLAY} -fg -geometry ${VNC_GEOMETRY} -depth ${VNC_DEPTH} -rfbport ${VNC_PORT} -xstartup ${HOME_DIR}/.vnc/xstartup
EOF
...
echo "[entrypoint] TurboVNC on ${DISPLAY} (port ${VNC_PORT}), noVNC on ${NO_VNC_PORT}, geometry ${VNC_GEOMETRY}, depth ${VNC_DEPTH}"
